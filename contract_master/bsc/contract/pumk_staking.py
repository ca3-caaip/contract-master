...
class PumkStaking(Contract):
    ...
    def __init__(
        self, web3: Web3, address: str, txs: list[CovalentTx] | None = None
    ) -> None:
    ...
    def balance_of(
        ...
        staked = self._get_staked_balance(
            account=account, block_identifier=block_identifier
        )
        reward = self._get_reward_balance(
            account=account, block_identifier=block_identifier
        )

        return [
            StakedServiceItem(
                data=StakedServiceItem.StakedServiceData(supply=[staked], reward=reward)
            )
        ]

    def _get_staked_balance(
        self, account: str, block_identifier: int | Literal["latest"]
    ) -> TokenAmount:
    ...
    def _get_reward_balance(
        ...
        equator_balance = EquatorLiquidityPool(
            web3=self.web3, address=token
        ).balance_of(account=account, block_identifier=block_identifier)
