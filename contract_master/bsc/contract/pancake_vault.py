...
class PancakeVault(Contract):
    ...
    def balance_of(
        ...
        token: str = self.contract.functions.token().call(
            block_identifier=block_identifier
        )
        token_balance: int = self.contract.functions.balanceOf().call(
            block_identifier=block_identifier
        )
        total_shares: int = self.contract.functions.totalShares().call(
            block_identifier=block_identifier
        )
        user_shares: int = self.contract.functions.userInfo(account).call(
            block_identifier=block_identifier
        )[0]
