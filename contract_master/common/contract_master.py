...
class ContractMaster(ABC):
    ...
    def __init__(
        ...
        self.txs: list[CovalentTx] = (
            filter_by_datetime_within(txs, target_datetime) if target_datetime else txs
        )
        self.block_identifier: int | Literal["latest"] = (
            get_max_block_height(txs) if target_datetime else "latest"
        )
...
def filter_by_datetime_within(
    txs: list[CovalentTx], time: datetime
) -> list[CovalentTx]:
