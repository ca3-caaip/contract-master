...
def parse_args():
    ...
    parser.add_argument(
        "--quicknode_endpoint", help="--quicknode_endpoint [quicknode_endpoint]"
    )
    ...
    return Arguments(
        ...
        or os.getenv("QUICKNODE_POLYGON_ENDPOINT", ""),
        target_datetime=args.target_datetime
        or None,  # e.g. datetime(2022, 9, 30, 1, 31, tzinfo=timezone.utc)
        txs_path=args.txs_path
        or os.path.join(
            os.path.dirname(__file__), "../sample_data/polygon/tx-0x283b7f.json"
        ),
    )
...
if __name__ == "__main__":
    ...
    if args.platform == "bsc":
        logger.info(
            "initialize BscContractMaster with loading transactions from"
            f" {args.txs_path}"
        )
    ...
    if args.platform == "polygon":
        logger.info(
            "initialize PolygonContractMaster with loading transactions from"
            f" {args.txs_path}"
        )
